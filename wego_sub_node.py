...
class LineTracerWithObstacleAvoidance:
    ...
    def lidar_cb(self, scan):
        ...
        # 정면 범위 (가운데 20개 데이터 추출)
        front_zone = np.concatenate([raw[:10], raw[-10:]])
        cleaned = [d for d in front_zone if d > 0.20 and not np.isnan(d) and not np.isinf(d)]
    ...
    def camera_cb(self, msg):
        ...
        if self.state == "LANE":
            ...
            try:
                ...
                red_mask = cv2.bitwise_or(cv2.inRange(hsv_near, lower_r1, upper_r1),
                ...
                if len(centers) >= 1:
                    centers.sort()
                    mid = (centers[0] + centers[-1]) // 2 if len(centers) >= 2 else centers[0]
                    error = mid - (w // 2)
                    twist.linear.x = 0.21
                ...
                if not contours_near:
                    ...
                    if M["m00"] != 0:
                        ...
                        twist.linear.x = 0.22
                        twist.angular.z = np.clip(gain * error, -self.max_steer, self.max_steer)
    ...
    def find_gap_max(self):
        ...
        ranges = np.where((ranges < 0.20) | np.isnan(ranges) | np.isinf(ranges), 0.0, ranges)
